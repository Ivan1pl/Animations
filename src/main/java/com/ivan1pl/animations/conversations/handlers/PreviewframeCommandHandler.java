 ...
 * @author Ivan1pl
 */
public class PreviewframeCommandHandler extends ConversationCommandHandler {
    
    private final Prompt successPrompt;
    
    private final Prompt failPrompt;
    
    public PreviewframeCommandHandler(Prompt successPrompt, Prompt failPrompt) {
        super("previewframe", 1, "frame_index");
    ...
    public Prompt handle(ConversationContext cc, Animation animation, String animationName, String[] params) throws AnimationTypeException {
        ...
        sAnimation.stop();
        if (sAnimation.showFrame(index)) {
            return new ConversationResponsePrompt(successPrompt, MessageUtil.formatInfoMessage(Messages.MSG_DISPLAYING_FRAME, index));
        } else {
